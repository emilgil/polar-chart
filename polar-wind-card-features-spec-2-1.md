...
- `show_max_wind` and `show_wind_rose` are **not** YAML keys — controlled via toggle buttons
- `view_mode` remains a YAML key (sets initial mode) and is also toggleable via button
...
Controlled via a **toggle button in the controls row** — no YAML key.
Initial state: off (`this._showMaxWind = false`).
...
4. **Subtle pulse animation:**
   ...
   - Implemented via `requestAnimationFrame` loop only when `_showMaxWind === true`
   - Animation period: 2 seconds
   - When toggled on: start animation loop
   - When toggled off: cancel animation loop, return to static `_redrawFromCache()`
...
```html
<button id="pw-maxwind" title="Maxvind / Max wind">⚡</button>
```

- Clicking toggles `this._showMaxWind`
- Button gets a CSS `active` class when enabled (e.g. slightly brighter background)
- No fetch on toggle — `_redrawFromCache()` only

### i18n for button title

```javascript
sv: { maxWindBtn: 'Visa maxvind' }
en: { maxWindBtn: 'Show max wind' }
```

### Acceptance criteria

- [ ] Toggle button visible in controls row
- [ ] Clicking button toggles `_showMaxWind` and updates button active state
- [ ] No YAML key needed
...
- [ ] No animation loop running when `_showMaxWind` is false
...
Controlled via a **toggle button in the controls row** — no YAML key.
Initial state: off (`this._showWindRose = false`).
...
```html
<button id="pw-windrose" title="Vindros / Wind rose">🌹</button>
```

- Clicking toggles `this._showWindRose`
- Button gets a CSS `active` class when enabled
- No fetch on toggle — `_redrawFromCache()` only

### i18n for button title

```javascript
sv: { windRoseBtn: 'Visa vindros' }
en: { windRoseBtn: 'Show wind rose' }
```

### Acceptance criteria

- [ ] Toggle button visible in controls row
- [ ] Clicking button toggles `_showWindRose` and updates button active state
- [ ] No YAML key needed
...
## Summary of new toggle buttons

| Button ID      | Icon | State variable    | Default |
|----------------|------|-------------------|---------|
| `pw-maxwind`   | ⚡   | `_showMaxWind`    | off     |
| `pw-windrose`  | 🌹   | `_showWindRose`   | off     |
| `pw-mode`      | 🔄   | `_viewMode`       | spiral  |

## Summary of new i18n strings

| Key           | sv                | en                |
|---------------|-------------------|-------------------|
| `maxWindBtn`  | Visa maxvind      | Show max wind     |
| `windRoseBtn` | Visa vindros      | Show wind rose    |
| `today`       | Idag              | Today             |
| `days`        | dygn              | days              |
